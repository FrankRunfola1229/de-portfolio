/**
 * sql.js
 * Renders snippets from assets/data/sql_snippets.json
 *
 * Depends on shared.js (window.Site)
 * Uses Prism to highlight after render.
 */

(async function initSqlPage() {
  const grid = document.getElementById("snippetsGrid");
  if (!grid) return;

  try {
    const items = await Site.fetchJson("./assets/data/sql_snippets.json");
    if (!Array.isArray(items)) throw new Error("sql_snippets.json must be an array");
